...
import { setupPassword } from '@/utils/passwordSetup';
...
export const PasswordSetupModal = ({ isOpen, onClose, onSuccess }: PasswordSetupModalProps) =>
{
    ...
    const handleSubmit = async (e: React.FormEvent) =>
    {
        ...
        if (isSubmitDisabled)
        {
            if (!validation.hasPassword || !validation.hasConfirmPassword)
            {
                setError('Please fill in all fields');
            } else if (!validation.meetsAllRequirements)
            {
        ...
        try
        {
            ...
            {
                setError(result.message || 'Failed to set up password');
            }
        } catch (error)
        {
